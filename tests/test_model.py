...
def test_sdxl_forward():
    ...
    print('in sdxl test')
    model = stable_diffusion_xl(pretrained=False, fsdp=False, encode_latents_in_fp16=False)
    model = model.to('cuda')
    batch_size = 1
    H = 8
    W = 8
    image = torch.randn(batch_size, 3, H, W).to('cuda')
    ...
    output, target, _ = model.forward(batch)  # model.forward generates the unet output noise or v_pred target.
